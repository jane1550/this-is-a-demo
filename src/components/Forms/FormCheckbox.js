...
export default class FormCheckbox extends React.Component {
  ...
  onChange(checked, optionValue) {
    let {disabled, readOnly, onChange} = this.props
  ...
  getDisplayText() {
    ...
    for (let i = 0; i < value.length; i++) {
      labels.push(getLabelByValue(options, value[i]))
    }
    return labels.join(' | ')
  }
    ...
    if (options && options.length > 0) {
      ...
      options.map((option, index) => {
        if (option && typeof (option.value) !== "undefined") {
          let checked = this.state.value.includes(option.value)
          optionElements.push(
            <div key={id + '_' + index} className="custom-control custom-checkbox m-2">
              <input type="checkbox" id={id + '_' + index} name={id} className={inputClass}
          ...
          let checked = this.state.value.includes(option)
          optionElements.push(
            <div key={id + '_' + index} className="custom-control custom-checkbox m-2">
              <input type="checkbox" id={id + '_' + index} name={id} className={inputClass}
                     value={option} checked={checked}
                     onChange={e => this.onChange(e.target.checked, option)}
                     onBlur={e => this.onBlur(e.target.checked, option)} onFocus={e => this.onFocus(e)}
                     onClick={e => this.onClick(e)} disabled={disabled}
